...
export function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <div className="group bg-card rounded-2xl overflow-hidden shadow-soft hover-lift border border-border flex flex-col">
      <div className={`relative overflow-hidden ${isCoffee ? "bg-gradient-to-br from-amber-50 to-stone-100 dark:from-stone-900 dark:to-stone-800 p-4" : "bg-secondary"} ${compact ? "aspect-[4/3]" : "aspect-square"}`}>
        <Link href={`/product/${product.slug}`}>
          <Image src={product.image} alt={product.title} width={400} height={400} loading="lazy"
            className={`w-full h-full ${isCoffee ? "object-contain group-hover:scale-105" : "object-cover group-hover:scale-110"} transition-transform duration-500`} />
        </Link>
        {product.badge && (
