...
const persist = async () => {
  ...
  // Sync to Supabase (except cart/wishlist which are session-based)
  try {
    if (state.orders.length > 0) await supabase.from('orders').upsert(state.orders);
    if (state.customers.length > 0) await supabase.from('customers').upsert(state.customers);
    if (state.offers.length > 0) await supabase.from('offers').upsert(state.offers);
    await supabase.from('settings').upsert([{ id: 'site_settings', ...state.settings }]);
  } catch (err) {
    console.error("Supabase Store Sync Error:", err);
  }
};

// Initial sync from Supabase
export const syncStoreFromSupabase = async () => {
  try {
    ...
    const { data: settings } = await supabase.from('settings').select('*').eq('id', 'site_settings').single();
    if (settings) {
      const { id, ...cleanSettings } = settings;
      state.settings = cleanSettings as SiteSettings;
    }
